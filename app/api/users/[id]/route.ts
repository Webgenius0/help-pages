...
export async function PUT(
  ...
  try {
    ...
    const { email, username, fullName, avatarUrl, bio, role, password, isPublic } = body
    ...
    if ((email || username) && !isAdmin) {
      return NextResponse.json({ error: 'Only admins can change email or username' }, { status: 403 })
    }
